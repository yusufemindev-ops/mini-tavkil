...
describe('buildMetadata hreflang alternates', () => {
  ...
  it('advertises every configured locale by default (static pages)', () => {
  ...
  it('advertises only the locales that have a real translation', () => {
    const languages =
      buildMetadata({ ...base, alternateLocales: ['en', 'tr'] }).alternates?.languages ?? {};
  ...
  it('keeps x-default on the en URL even for an en-only entity', () => {
    const languages =
      buildMetadata({ ...base, alternateLocales: ['en'] }).alternates?.languages ?? {};
  ...
  it('ignores unknown locale codes', () => {
    const languages =
      buildMetadata({ ...base, alternateLocales: ['en', 'de', 'fr'] }).alternates?.languages ?? {};
    expect(Object.keys(languages).sort()).toEqual(['en', 'x-default']);
  });

  it('points each alternate at that locale’s own slug', () => {
    const languages =
      buildMetadata({
        ...base,
        localizedPaths: {
  ...
  it('canonicalises to the page’s own locale, never cross-locale', () => {
...
describe('buildMetadata robots', () => {
  ...
  it('emits noindex,follow for a page flagged not-indexable', async () => {
    const { buildMetadata } = await loadWith(true);
    expect(buildMetadata({ ...base, index: false }).robots).toEqual({ index: false, follow: true });
  });
  ...
  it('forces noindex on every page while SITE_INDEXABLE is false', async () => {
    ...
    expect(buildMetadata({ ...base, index: true }).robots).toEqual({ index: false, follow: true });
  });
});
