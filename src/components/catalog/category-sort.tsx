'use client';

import { useTranslations } from 'next-intl';
import { PRODUCT_SORTS, type ProductSort } from '@/lib/catalog/product-sort';

// Labels for the sort values the query layer supports. Keyed off PRODUCT_SORTS so
// adding a sort there without a label here is a type error rather than a blank
// option. There is no price sort: there is no public price to sort by.
const LABEL_KEY: Record<ProductSort, string> = {
  recommended: 'sort_recommended',
  moq: 'sort_moq',
  az: 'sort_az',
};

// A controlled `<select>`. The order it selects and the URL it is reflected in
// are both owned by `CategoryListing`, which holds the sort state — this only
// reports the choice, so there is one place that decides what sorting means.
export function CategorySort({
  value,
  onChange,
}: {
  value: ProductSort;
  onChange: (next: ProductSort) => void;
}) {
  const t = useTranslations('store');

  return (
    <label className="text-muted-foreground flex items-center gap-2 text-sm">
      {t('sort_by')}
      <select
        value={value}
        onChange={(event) => onChange(event.target.value as ProductSort)}
        className="border-input bg-background text-foreground focus-visible:border-primary cursor-pointer rounded-sm border px-2.5 py-1.5 text-sm outline-none"
      >
        {PRODUCT_SORTS.map((sort) => (
          <option key={sort} value={sort}>
            {t(LABEL_KEY[sort])}
          </option>
        ))}
      </select>
    </label>
  );
}
